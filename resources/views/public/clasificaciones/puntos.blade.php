...
@section('title', 'Clasificación por puntos — ' . $prueba->nombre)
...
@section('content')
    ...
    <flux:breadcrumbs class="mb-6">
        ...
        <flux:breadcrumbs.item>Clasificación por puntos</flux:breadcrumbs.item>
    </flux:breadcrumbs>

    {{-- Cabecera --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            <flux:heading size="xl" level="1" accent>Clasificación por puntos</flux:heading>
        ...
        <div class="flex items-center gap-2">
            ...
            <flux:button variant="primary" size="sm" color="emerald">Puntos</flux:button>
                ...
                <flux:table.rows>
                    @foreach ($clasificacion as $item)
                        <flux:table.row key="{{ $item['ciclista']->id }}">
                            ...
                            {{-- Puntos --}}
                            <flux:table.cell align="center">
                                <flux:text size="xs" class="font-mono tabular-nums" variant="strong">
                                    {{ $item['total_puntos'] }}
                                </flux:text>
                            </flux:table.cell>
                        </flux:table.row>
    ...
    @else
        <div class="text-center py-24">
            <flux:heading size="lg">No hay puntos registrados todavía para esta prueba.</flux:heading>
        </div>
    @endif
